...
export const MetricCard = ({ eyebrow, title, value, valueSubheading, subtitle, trend, privacyMode }) => {
  ...
  return (
    <div className="flex h-full min-h-[118px] flex-col rounded-md border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2">
      ...
      <p className={`mt-0.5 text-base leading-tight font-semibold text-slate-900 dark:text-gray-100 ${blurClass}`}>{value}</p>
      {valueSubheading ? (
        <p className="mt-0.5 text-[11px] leading-tight text-slate-600 dark:text-gray-400">{valueSubheading}</p>
      ) : null}
      <div className="mt-auto space-y-0.5">
        <p className={`min-h-[14px] text-[11px] leading-tight text-slate-600 dark:text-gray-400 ${blurClass}`}>
          {subtitle || " "}
        </p>
