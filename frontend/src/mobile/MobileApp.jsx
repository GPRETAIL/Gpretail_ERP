...
import { BarChart3, Bell, Box, ChevronLeft, ChevronRight, ClipboardList, FileBarChart, Home, Menu, MoreHorizontal, Package, Plus, Search, Settings, ShoppingCart, Sparkles, Store, Users, Wallet, X } from "lucide-react";
import "./mobile.css";
...
const modules = [
  ["sales", "Sales", ShoppingCart], ["purchase", "Purchase", ClipboardList], ["inventory", "Inventory", Box],
...
export default function MobileApp() {
  ...
  return <div className="vx-mobile-app">
    <header className="vx-mobile-topbar">
      <button className="vx-icon-btn vx-mobile-only" onClick={() => setMobileMenuOpen(true)} aria-label="Open menu"><Menu size={20} /></button>
      <div className="vx-brand-lockup"><div className="vx-brand-mark">V</div><span>Vynerix ERP</span></div><div className="vx-topbar-spacer" />
      <button className="vx-icon-btn" aria-label="Notifications"><Bell size={19} /></button><div className="vx-avatar">{userName.charAt(0).toUpperCase()}</div>
    ...
    <div className="vx-app-body">
      <aside className={`vx-sidebar ${mobileMenuOpen ? "is-open" : ""}`}>
        <div className="vx-sidebar-head"><div><small>Workspace</small><strong>GP Retails</strong></div><button className="vx-icon-btn vx-mobile-only" onClick={() => setMobileMenuOpen(false)} aria-label="Close menu"><X size={20} /></button></div>
        <nav className="vx-side-nav">{modules.map(([key, label, Icon]) => <button key={key} className={`vx-side-link ${page === key ? "active" : ""}`} onClick={() => navigate(key)}><Icon size={18} /><span>{label}</span></button>)}</nav>
        <div className="vx-install-card"><Sparkles size={18} /><div><strong>Install Vynerix ERP</strong><span>Use it like a mobile app</span></div>{installEvent && <button onClick={install}>Install</button>}</div>
      </aside>
      <main className="vx-main">
        <div className="vx-mobile-pagebar">{page !== "dashboard" && <button className="vx-icon-btn" onClick={() => navigate("dashboard")} aria-label="Back"><ChevronLeft size={18} /></button>}<div><small>{formatNow()}</small><h1>{currentTitle}</h1></div>{page !== "dashboard" && <button className="vx-icon-btn" onClick={() => navigate("dashboard")} aria-label="Dashboard"><Home size={18} /></button>}</div>
    ...
    <nav className="vx-bottom-nav">{navItems.map(({ key, label, icon: Icon }) => <button key={key} className={page === key || (key === "more" && ["products", "customers", "suppliers", "expenses", "reports", "settings"].includes(page)) ? "active" : ""} onClick={() => key === "more" ? setMobileMenuOpen(true) : navigate(key)}><Icon size={19} /><span>{label}</span></button>)}</nav>
    {showCreateInvoice && <CreateInvoice onClose={() => setShowCreateInvoice(false)} />}
  </div>;
}

function Dashboard({ userName, onNavigate }) { return <div className="vx-page-grid">
  <section className="vx-hero-card"><div><small>Good morning</small><h2>Hello, {userName}</h2><p>Everything you need for today's business.</p></div><div className="vx-date-pill">19 Aug · 19 Aug 2025 <ChevronRight size={14} /></div></section>
  <section className="vx-kpi-grid"><Kpi label="Sales Today" value="₹ 28,450" trend="+12.9%" tone="green" /><Kpi label="Purchase Today" value="₹ 18,750" trend="-4.3%" tone="red" /><Kpi label="Receivables" value="₹ 1,25,000" tone="indigo" /><Kpi label="Payables" value="₹ 75,400" tone="violet" /></section>
  <section className="vx-card"><div className="vx-section-head"><div><h3>Sales Overview</h3><span>This week</span></div><button className="vx-filter-btn">This Week <ChevronRight size={14} /></button></div><div className="vx-chart"><div className="vx-axis"><span>30K</span><span>20K</span><span>10K</span><span>0</span></div><svg viewBox="0 0 520 180" role="img" aria-label="Sales overview"><polyline points="20,140 90,115 160,120 230,75 300,95 370,85 450,35" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" /><line x1="0" y1="160" x2="510" y2="160" stroke="currentColor" opacity=".15" /><g fill="currentColor"><circle cx="20" cy="140" r="5"/><circle cx="90" cy="115" r="5"/><circle cx="160" cy="120" r="5"/><circle cx="230" cy="75" r="5"/><circle cx="300" cy="95" r="5"/><circle cx="370" cy="85" r="5"/><circle cx="450" cy="35" r="5"/></g></svg></div></section>
