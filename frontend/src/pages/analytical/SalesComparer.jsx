...
const SalesComparer = () => {
  ...
  const columns = useMemo(() => [
    ...
    { key: "total_discount", label: "Discount", render: (v) => numericCell(fmt(v), { color: "warning.main" }) },
      ...
      <Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between", px: 2, py: 1, bgcolor: "background.paper", borderBottom: 1, borderColor: "divider", boxShadow: 1 }}>
        <Stack direction="row" spacing={1} sx={{ alignItems: "center" }}>
          ...
          <Stack direction="row" spacing={0.5} sx={{ alignItems: "center", fontSize: 13, fontWeight: 600 }}>
            ...
            <Box component="span">Sales Comparer</Box>
          </Stack>
        </Stack>
      </Stack>
      ...
      <Stack direction="row" spacing={2} sx={{ flex: 1, minHeight: 0, p: 2 }}>
        ...
        >
          <Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between", mb: 0.75 }}>
            <Box>
              <Typography component="h2" sx={{ fontSize: 14, fontWeight: 700, color: "text.primary" }}>Sales Comparer</Typography>
              <Typography sx={{ fontSize: 10.5, color: "text.secondary", mt: 0.25 }}>
                ...
                {saleFromDate || saleToDate ? (
                  <Box component="span">
                    {" "}• Sale Range: <Box component="span" sx={{ fontWeight: 500, color: "#6366f1" }}>{saleFromDate || "Any"}</Box>
                    {" "}to <Box component="span" sx={{ fontWeight: 500, color: "#6366f1" }}>{saleToDate || "Any"}</Box>
                  </Box>
                ) : null}
          ...
          {totals && !notice && (
            ...
            >
              ...
              <Box component="span">Sale Amt: {fmt(totals.net_amount)}</Box>
              <Box component="span">Cost Amt: {fmt(totals.net_cost)}</Box>
              <Box component="span" sx={{ color: "warning.main" }}>Discount: {fmt(totals.total_discount)}</Box>
