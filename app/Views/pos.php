...
?>
<input type="hidden" name="retunbill" id="retunbill" value="2">
...
?>
<?php
// $mmik = $db->query("SELECT * FROM products ORDER BY id DESC LIMIT 100")->getRowArray();
$mmik = $ProductModel->asArray()->orderBy('id', 'DESC')->first();


$user->id;
$pll = $db->query("select * from users where id='" . $user->id . "' ")->getRowArray();


$rolwer = $user->role;
@$kkar = $db->query("SELECT * FROM permission_new WHERE nname='$rolwer'")->getRowArray();
                ...
                <ul id="storeline">
                    <?php foreach ($Stores as $store) : ?>
                        <a href="javascript:void(0)"
                            onclick="OpenRegister(<?= $store->status ? $store->status : 0 ?>, <?= $store->id ?>)">
                            <li class="listing clearfix">
                                <div class="image_wrapper">
                                    <img src="<?= base_url() ?>/public/assets/img/store.svg" alt="store">
                                </div>
                                <div class="info">
                                    <span class="store_title"><?= $store->name ?></span>
                                    <span class="store_info"><?= $store->city ?> <span>&bull;</span> <?= $store->phone ?>
                                        <span>&bull;</span> <?= $store->email ?></span>
                                </div>
                                <span
                                    class="store_type <?= $store->status == 1 ? 'store_open' : 'store_close' ?>"><?= $store->status == 1 ? label('open') : label('close') ?></span>
                            </li>
                        </a>
                    <?php endforeach; ?>
                </ul>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="number" step="any" name="cash" Required class="form-control" id="CashinHand"
                            placeholder="<?= label('CashinHand') ?>">
...
<?php
    ...
    <div class="container-fluid">
        <div class="row" style="margin-left: -10px;">
            <input type="hidden" name="pprtnt" id="pprtnt" value="<?php echo $setting->ddirectprint; ?>">
            <div class="col-md-8 left-side " style="margin-bottom: 1px;padding-right: 5px;padding-left: 5px;">
                <div class="row">
                    <div class="col-xs-6">
                        <div class="row row-horizon" style="margin: 0 0px;">
                            ...
                            <span data-toggle="tooltip" title="Remove Selected" class="Hold rl"
                                onclick="RemoveHold()">-</span>
                        </div>
                    </div>
                    <div class="col-xs-6 " style="margin-top:-3px;text-align: right;">
                        <?php if ($setting->quotation == 1) { ?>
                            <input type="text" name="qttnumm" id="qttnumm" placeholder="QT No"
                                style="width: 60px;border: 1px solid #C8D3DF;
                        ...
                        <?php
                        if ((session()->has('payment_price'))) {
                            $sellrr = session()->get('payment_price');
                        } else {
                            $sellrr = 0;
                        }
                        ?>
                        <?php
                        if ($setting->mlp_rss == 1) {
                        ?>
                            <span style="width: 25%;float: right;margin: 2px;">
                                <select class="form-control" id="payment_price"
                                    style="width: 100%;height: 40px;float: right; display: block;padding: 2px;
                                        line-height: 2.42857143;
                                            background: linear-gradient(to bottom, #ffffff 0%,#eeeeee 100%);
                                        color: #000;
                                        text-align: center;
                                        border: 1px solid #C8D3DF;">
                                    <option value="0" <?php if ($sellrr == 0) { ?> selected="selected" <?php } ?>>
                                        <?= label('Store') ?></option>
                                    <?php
                                    $builder = $db->table('price_master'); // Connect to the 'price_master' table
                                    $pprrc = $builder->orderBy('name', 'asc')->get(); // Order by 'name' in ascending order

                                    // Check if there are any results
                                    if ($pprrc->getNumRows() > 0) {
                                        foreach ($pprrc->getResult() as $pprrcf) {
                                    ?>
                                            <option value="<?= $pprrcf->id ?>" <?= $sellrr == $pprrcf->id ? 'selected="selected"' : '' ?>>
                                                <?= $pprrcf->name ?>
                                            </option>
                                    <?php
                                        }
                        ...
                        <a href="javascript:void(0)" onclick="showlastbill()">
                            <span class="categories"><i class="fa fa-tags" aria-hidden="true"></i>

                                <?= label('Receipt') ?>
                            </span></a>
                        <a data-toggle="tooltip" href="javascript:void(0)" onclick="CloseRegister()"><span
                                class="categories">
                ...
                <script type="text/javascript">
                    function showlastbill() {
                        $.ajax({
                            url: "<?php echo site_url('invoices/ShowTicket/0'); ?>",
                            type: "POST",
                ...
                <div class="col-sm-12"
                    style="height: 53px;margin-top: 4px;margin-bottom: 5px;border: solid 1px #CFCFCF;
                    ...
                    if ($segment_3) {
                        $qt_id = $segment_3;
                        $ssel_cust = $db->query("SELECT * from saleqs where id='" . $segment_3 . "' ")->getRow();
                    ...
                    <div class="col-sm-3" style="margin-top: 10px;padding-right: 1px;padding-left: 0px;">
                        ...
                        <select class="js-select-options form-control" id="customerSelect">
                            ...
                            <?php foreach ($customers as $customer) : ?>
                                <option value="<?= $customer->id ?>" <?php if ($ssel_custf == $customer->id) {
                                                                            $ssel_custmob = $customer->phone; ?> selected <?php } ?>>
                                    <?php echo $customer->name . ' - ' . $customer->phone; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="col-sm-4" style="padding-left: 0px;padding-right:  0px;">
                        <form onsubmit="return barcode()" action="javascript:void(0)">
                            <input autofocus style="margin-top:10px;" placeholder="<?= label('Barcode_Product_Name') ?>"
                                autocomplete="off" onkeyup="auromcv(this.value,this.id);" class="form-control barcode"
                                type='text' id='countryname_1m' />

                            <!-- <input  style="margin-top:10px;" type="text" autofocus id="<?php //echo strval($setting->keyboard) === '1' ? 'keyboard' : '' 
                                                                                            ?>" class="form-control barcode" placeholder="<?= label('BarcodeScanner') ?>"> -->
                        </form>
                    ...
                    <div class="col-sm-3" style="margin-top: 15px;font-size: 19px;">
                        <span class="float-left"><i style="margin-right:5px; color:#89b03e;font-size: 20px;"
                                class="fa fa-shopping-cart"></i><b id="ItemsNum"><span></span>
                                <?= label('item') ?></b></span>
                    </div>
                    <span title="Add New Product"
                        style="cursor: pointer;margin-top: 13px;font-size: 25px;color: #89b03e !important;"
                        class="float-left" data-toggle="modal" data-target="#Addproduct">
                        <i class="fa fa-plus"></i></span>
                </div>
                <div class="clearfix"></div>
                <div class="row"
                    style="background: #FBFBF7;border:1px solid #CFCFCF;padding-left:1px;font-family:'Roboto', sans-serif;">

                    <div class="col-lg-1 table-header" style="padding-right: 5px;width: 5%;  padding-left: 5px;">
                        <h5 style="margin-top:10px;text-align: left;"><b><?= label('PID') ?></b></h5>
                    </div>

                    <div class="col-xs-3 table-header" style="padding-right: 5px;padding-left: 5px;width: 25%;">
                        <h5 style="margin-top:10px;text-align: center;"><b><?= label('Product') ?></b></h5>
                    </div>
                    <div class="col-lg-1 table-header" style="padding-right: 5px;padding-left: 5px;min-width: 10%; text-align:center;">
                        <h5 style="margin-top:10px;text-align: right;"><b>MRP</b></h5>
                    </div>

                    <div class="col-lg-1 table-header" style="padding-right: 5px;padding-left: 5px;min-width: 10%;">
                        <h5 style="margin-top:10px;text-align: center;"><b><?= label('Price') ?></b></h5>
                    </div>
                    <div class="col-lg-1 table-header " style="padding-right: 1px;padding-left: 5px;min-width: 10%;">
                        <h5 style="margin-top:10px;text-align: center;margin-right: 10px;"><b><?= label('Quantity') ?></b>
                        </h5>
                    </div>
                    <?php
                    ...
                    if ($setting->disc_pro == 1) {
                        $msk--;
                    ?>
                        <div style="padding-right: 1px;padding-left: 1px;width: 10%;"
                            class="col-lg-1 table-header ">
                            <input type="hidden" name="prowise" id="prowise" value="1">
                            <h5 style="margin-top:10px; font-weight: bold; margin-left: 10%;">Dis %</h5>
                        </div>
                        <div style="padding-right: 1px;padding-left: 1px;width: 10%;"
                            class="col-lg-1 table-header ">
                    ...
                    ?>
                        ...
                        <div style="text-align: left;display:none;width: 10%;" class=" table-header ">
                    ...
                    if ($setting->gst_tax == 1) {
                        $msk--;
                        if (isset($customer->custstate) && $setting->mystate == $customer->custstate || !empty($_SESSION['custkid']) && $_SESSION['custkid'] == 0) { ?>
                        ...
                        <div style="text-align: left;display:none;" class=" table-header ">
                            <h3 style="margin-top:10px;" class="text-left">&nbsp;</h3>
                        </div>
                        <div style="text-align: left;display:none;" class=" table-header ">
                            <h3 style="margin-top:10px;" class="text-left">&nbsp;</h3>
                    ...
                    ?>
                    <div class="col-lg-1 table-header nopadding " style="width: 5%;">
                        <h5 style="margin-top:10px;text-align: center;"><b><?= label('Total') ?></b></h5>
                    </div>
                    <div class="col-lg-1 table-header nopadding" style="text-align: center;width: 10%;">
                        <i class="fa fa-question-circle-o" title="If uncheck,discount won't calculate for that product" aria-hidden="true"></i>
                    </div>
                ...
                ?>
                    <div class="footer-section">
                        <div class="table-responsive col-sm-12 totalTab">
                            ...
                            <table class="table"
                                style="border: 1px solid #ddd !important;    margin-top: 6px;margin-bottom: 5px !important;">

                                <tr>


                                    <?php if ($setting->disc_all == 1) { ?>
                                        <input type="hidden" name="allddd" id="allddd" value="1">
                                        <td class=""
                                            style="text-align: center; width:12%;padding: 5px;font-family:'Roboto', sans-serif;border: 1px solid #ddd !important;">
                                            <b><?= label('Discount') ?></b> (%)
                                            <input maxlength="5"
                                                style="width: 100%;height: 30px;float: right;background-color: #fff;display: block;margin-top: 5px;padding: 6px 10px;
                                                ...
                                                type="text" value="<?= $ssel_custds ?>" onkeyup="total_change()"
                                                id="<?= strval($setting->keyboard) === '1' ? 'num02' : '' ?>"
                                                class="total-input Remise " placeholder="N/A">
                                    ...
                                    <?php } else {  ?>
                                        <td class=""
                                            style="width:12%;padding: 14px;font-family:'Roboto', sans-serif;border: 1px solid #ddd !important;">
                                            &nbsp;
                                            <input type="hidden" name="allddd" id="allddd" value="0">
                                            <input type="hidden" value="0" onchange="total_change()"
                                                id="<?= strval($setting->keyboard) === '1' ? 'num02' : '' ?>"
                                                class="total-input Remise" placeholder="N/A" maxlength="5">
                                    ...
                                    <td class=""
                                        style="padding: 5px;text-align: center; font-family:'Roboto', sans-serif;border: 1px solid #ddd !important;"
                                        width="13%">
                                        <b><?= label('Shipping') ?></b>

                                        <input maxlength="10"
                                            style="width: 100%;height: 30px;float:right;background-color:#fff;display: block;padding: 5px;
                                                    ...
                                                    transition: border-color ease-in-out .15s,box-shadow ease-in-out .15s;"
                                            type="text" value="" onkeyup="total_changeddish()" id="disamtssh"
                                            class="total-input" placeholder="Shipping">
                                    </td>
                                    ...
                                    <td class=""
                                        ...
                                        $payy = $db->query("select * from payment_mode where status=1 order by id asc ")->getResult();
                                        foreach ($payy as $payyf) { ?>
                                            <input type="hidden" id="payment_<?php echo $payyf->id; ?>" value="<?php echo $payyf->validate_it; ?>">
                                        <?php } ?>


                                        <b><?= label('payment_mode') ?> </b>
                                        ...
                                        <select class="form-control" id="paymentMethod"
                                            ...
                                            $payy = $db->query("SELECT * FROM payment_mode ORDER BY id ASC ")->getResult();
                                    ...
                                    <td class=""
                                        style="padding: 5px; font-family:'Roboto', sans-serif;border: 1px solid #ddd !important;"
                                        width="25%">

                                        <div class="form-group CreditCardNum">

                                            <input type="text" class="form-control cc-num" id="CreditCardNum"
                                                placeholder="<?= label('CreditCardNum') ?>">
                                        </div>
                                        <div class="clearfix"></div>
                                        <div class="form-group CreditCardHold  padding-s"
                                            style="margin: 0 0 0px 0 !important;">
                                            <input type="text" class="form-control" id="CreditCardHold"
                                                placeholder="<?= label('CreditCardHold') ?> Name">
                                        </div>
                                        ...
                                        <div class="col-xs-6 form-group ChequeNum nopadding"
                                            style="margin: 0 0 0px 0 !important;text-align: center;">
                                            <b> Ref Num </b><br>
                                            <input type="text" name="chequenum" class="form-control"
                                                placeholder="Ref Number" id="ChequeNum">
                                        </div>


                                        <div class="col-xs-6 ChequeNum table-header nopadding " style="text-align: center;">
                                            <b> Paid </b><br>
                                            <input onkeyup="return callcc2(this.value,this.id);" value="0"
                                                style="margin-left: 3px;text-align: center;" type="text"
                                                name="recivedamt2" class="form-control" placeholder="Amount"
                                                id="recivedamt2">
                                        </div>
                                    ...
                                    <td class=""
                                        style="padding: 5px;text-align: center; font-family:'Roboto', sans-serif;border: 1px solid #ddd !important;"
                                        width="15%"><b><?= label('paidcash') ?>(F7)</b>
                                        <input
                                            style="margin-top: 5px;width: 100%;height: 30px;float: right;background-color: #fff;display: block;padding: 5px;
                                                    line-height: 1.42857143;
                                                    color: #555;
                                                    text-align: center;
                                                    background-color: #fff;
                                                    ...
                                                    -o-transition: border-color ease-in-out .15s,box-shadow ease-in-out .15s;
                                                    transition: border-color ease-in-out .15s,box-shadow ease-in-out .15s;"
                                            type="text" value="0" onkeyup="return callcc(this.value,this.id);"
                                            name="recivedamt"
                                            class="form-control <?= strval($setting->keyboard) === '1' ? 'paidk' : '' ?>"
                                            id="recivedamt">
                                ...
                                <tr style=" ">
                                    <td
                                        style="width:10%; text-align: center; padding: 5px;font-family:'Roboto', sans-serif;border: 1px solid #ddd !important;">
                                        <b><?= label('Discount_amt') ?> </b>
                                        <?php
                                        if ($setting->disc_all == 1) {
                                        ?>

                                            <input maxlength="10"
                                                style="width: 100%;height: 30px;float: right;background-color: #fff;display: block;padding: 6px 10px;
                                                    line-height: 1.42857143;
                                                    color: #555;
                                                    text-align: center;
                                                    background-color: #fff;
                                                    ...
                                                    -o-transition: border-color ease-in-out .15s,box-shadow ease-in-out .15s;
                                                    transition: border-color ease-in-out .15s,box-shadow ease-in-out .15s;"
                                                type="text" value="<?= $setting->discount ?>"
                                                onkeyup="total_changeddi()" id="disamtss" class="total-input  "
                                                placeholder="N/A">
                                        ...
                                        <?php } else { ?>
                                            <span style="display:none" class="float-left"><b
                                                    id=""><span></span></b></span>
                                    ...
                                    <td class=""
                                        style="padding: 5px; text-align: center; font-family:'Roboto', sans-serif;border: 1px solid #ddd !important;">
                                        <b><?= label('Mobile') ?></b>
                                        <input value="<?php echo isset($ssel_custmob) ? $ssel_custmob : 0; ?>"
                                            style="width: 100%;height: 30px;float: right;background-color: #fff;display: block;padding: 6px 10px;
                                                    line-height: 1.42857143;
                                                    color: #555;
                                                    text-align: center;
                                                    ...
                                                    -o-transition: border-color ease-in-out .15s,box-shadow ease-in-out .15s;
                                                    transition: border-color ease-in-out .15s,box-shadow ease-in-out .15s;"
                                            type="text" name="mobnnm" class="-control" id="mobnnm" />
                                    </td>
                                    <td class=""
                                        style="padding: 5px; text-align: center;font-family:'Roboto', sans-serif;border: 1px solid #ddd !important;">
                                        <b><?= label('SubTotal') ?></b><br>
                                        <b><span style="margin-right: <?php echo $msk * 7; ?>%" class="float-right"><span
                                                    id="Subtot"></span></b>
                                    ...
                                    <td style="padding: 5px;border: 1px solid #ddd !important;color:red;" class="  text-bold">
                                        ...
                                        <span style="margin-right: <?php echo $msk * 7; ?>%;font-size: 32px;color: #0052ad;"
                                            class="float-right"><i style="font-size: 25px;" class="fa fa-rupee"></i> <b
                                                id="total"></b></span>
                                    </td>


                                    <td style="padding: 5px;text-align: center;">
                                        <b style="padding: 5px;"><?= label('Balanceamt') ?></b>
                                        <h3 style="color:red;margin-top: 0px;" id="bamacee"><i style="font-size: 25px;"
                                                class="fa fa-rupee"></i> <span></span> </h3>
                                    </td>
                            ...
                            ?>
                                ...
                                <input type="hidden" value="<?= $setting->tax ?>" onchange="total_change()"
                                    id="<?= strval($setting->keyboard) === '1' ? 'num01' : '' ?>" class="total-input TAX"
                                    placeholder="N/A" maxlength="5">

                                <span hidden style="margin-right: <?php echo $msk * $msm * 40; ?>px;" class="float-right">Rs.<b
                                        id="cgstt"></b></span>
                        ...
                        <button style="width: 21%;margin-left: 1.5%;border: 1px solid #B7B7B7 !important;"
                            class="btn btn-red col-md-6 flat-box-btn waves-effect waves-button" type="button"
                            onclick="cancelPOS()" class="btn categories col-md-4 ">
                        ...
                        <button onclick="retunbill(1);"
                            style="width: 21%;margin-left: 0.4%;border: 1px solid #B7B7B7 !important;" type="button"
                            class="btn btn-green col-md-4 flat-box-btn " data-toggle="modal" data-target="#AddSale">
                            <h4 class="text-bold"><i style="color:#fff;" class="fa fa-money"></i> <?= label('Return') ?></h4>
                        </button>

                        <button onclick="retunbill(2);"
                            style="width: 21%;margin-left: 0.4%;border: 1px solid #B7B7B7 !important;" type="button"
                            class="btn btn-green col-md-4 flat-box-btn " data-toggle="modal" data-target="#AddSale">
                            <h4 class="text-bold"><i style="color:#fff;" class="fa fa-money"></i> <?= label('Exchange') ?>
                            </h4>
                        </button>


                        <a onclick="saleBtn(1)" style="width: 33%;margin-left: 0.4%;border: 1px solid #B7B7B7 !important;"
                            href="javascript:void(0)" class="btn btn-green col-md-4 flat-box-btn ssf">
                            <h4><i style="color:#fff;" class="fa fa-print"></i> <?= label('Quick Pay & Print') ?> (F8)</h4>
                        </a>
                ...
                ?>
                    ...
                    <div class="footer-section">
                        ...
                        <div class="table-responsive col-sm-12 totalTab" style="padding-right: 3px; padding-left: 3px;">
                            ...
                            <table class="table"
                                style="border: 0px solid #ddd !important;    margin-top: 6px;margin-bottom: 3px !important;">
                                <tr style=" ">
                                    <?php if ($setting->disc_all == 1) { ?>
                                        <input type="hidden" name="allddd" id="allddd" value="1">
                                        <td class=""
                                            style="width:50%;padding: 5px;font-family:'Roboto', sans-serif;border: 0px solid #ddd !important;">
                                            <b><?= label('Discount') ?></b> (%)
                                            <input maxlength="5"
                                                style="width: 100%;height: 35px;float: right;background-color: #fff;display: block;margin-top: 5px;
...
transition: border-color ease-in-out .15s,box-shadow ease-in-out .15s;"
                                    ...
                                    <?php } else {  ?>
                                        <td class=""
                                            style="width:50%;padding: 14px;font-family:'Roboto', sans-serif;border: 0px solid #ddd !important;">
                                            &nbsp;
                                            <input type="hidden" name="allddd" id="allddd" value="0">
                                            <input type="hidden" value="0" onchange="total_change()"
                                                id="<?= strval($setting->keyboard) === '1' ? 'num02' : '' ?>"
                                    ...
                                    <td
                                        ...
                                        ?>
                                            ...
                                            <input maxlength="10"
                                                style="width: 100%;height: 35px;float: right;background-color: #fff;display: block;padding: 6px 10px;
line-height: 1.42857143;
color: #555;
text-align: center;
background-color: #fff;
...
transition: border-color ease-in-out .15s,box-shadow ease-in-out .15s;"
                                ...
                                <tr style=" ">
                                    <td class=""
                                        style="padding: 5px; font-family:'Roboto', sans-serif;border: 0px solid #ddd !important;"
                                        width="18%">
                                        <b><?= label('Shipping') ?></b>
                                        <input maxlength="10"
                                            style="width: 100%;height: 35px;float: right;background-color: #fff;display: block;padding: 5px;
line-height: 1.42857143;
color: #555;
text-align: center;
background-color: #fff;
...
transition: border-color ease-in-out .15s,box-shadow ease-in-out .15s;"
                                ...
                                <tr>
                                    <td class=""
                                        ...
                                        <select class="form-control" id="paymentMethod"
                                            ...
                                            $payy = $db->query("SELECT * FROM payment_mode WHERE status=1 AND id!=1  ORDER BY id ASC ")->getResult();
                                            foreach ($payy as $payyf) { ?>
                                                <option value="<?= $payyf->id ?>"><?= $payyf->name ?></option>
                                            <?php } ?>
                                        </select>
                                ...
                                <tr>
                                    ...
                                    <td style="padding: 5px;border: 0px solid #ddd !important;color:#0052ad;"
                                        class="  text-bold">
                                        <b><?= label('Total') ?></b>
                                ...
                                <tr>
                                    ...
                                    <td class=""
                                        style="padding: 5px; font-family:'Roboto', sans-serif;border: 0px solid #ddd !important;"
                                        width="20%"><b><?= label('paidcash') ?>(F7)</b>
                                        <input
                                            style="margin-top: 5px;width: 100%;height: 35px;float: right;background-color: #fff;display: block;padding: 5px;
                                                line-height: 1.42857143;
                                                color: #555;
                                                text-align: center;
                                                background-color: #fff;
                                    ...
                                    <td style="padding: 5px;text-align: left;border-top: 0px solid #ddd;">
                                        ...
                                        <h3 style="color:red;margin-top: 0px;text-align: right;" id="bamacee"><i
                                                style="font-size: 25px;" class="fa fa-rupee"></i> <span></span> </h3>
                                    </td>
                                </tr>
                                ...
                                ?>
                                    ...
                                    <input type="hidden" value="<?= $setting->tax ?>" onchange="total_change()"
                                        id="<?= strval($setting->keyboard) === '1' ? 'num01' : '' ?>"
                                        class="total-input TAX" placeholder="N/A" maxlength="5">
                        ...
                        <button style="width: 45%;margin-left: 3%;margin-top: 4%;border: 1px solid #B7B7B7 !important;"
                            class="btn btn-red col-md-6 flat-box-btn waves-effect waves-button" type="button"
                            onclick="cancelPOS()" class="btn categories col-md-4 ">
                        ...
                        <button style="width: 45%;margin-left: 3%;margin-top: 4%;border: 1px solid #B7B7B7 !important;"
                            type="button" class="btn btn-green col-md-4 flat-box-btn " data-toggle="modal"
                            data-target="#AddSale">
                            <h4 class="text-bold"><i style="color:#fff;" class="fa fa-money"></i> <?= label('Exchange') ?>
                            </h4>
                        </button>
                        ...
                        <a onclick="saleBtn(1)"
                            style="width: 56%;margin-left: 24%;margin-top: 4%;border: 1px solid #B7B7B7 !important;"
                            href="javascript:void(0)" class="btn btn-green col-md-4 flat-box-btn ssf">
                            <h4><i style="color:#fff;" class="fa fa-print"></i> <?= label('Quick Pay & Print') ?> (F8)</h4>
                        </a>
                ...
                ?>
                    ...
                    <div class="row row-horizon">
                        <span class="categories selectedGat" id=""><i style="font-size:18px;"
                    ...
                    <div id="productList2"
                        style="padding-left: 2px;padding-right: 10px;font-family: 'Roboto slab', serif;margin-top: 2px;font-weight: 600;">
                    ...
                    if ($setting->destpp == 0) { ?>
                            ...
                            <div class="col-sm-12" style="padding-right: 6px;padding-left: 1px;">
                                <div id="searchContaner">
                                    <div class="input-group stylish-input-group" style="width:100%;">
                                        <input type="text" id="searchProd" style="border: 1px solid #ccc;"
                        ...
                        ?>
                            ...
                            <div id="productList2"
                                style="padding-left: 2px;padding-right: 10px;margin-top: 2px;font-weight: 600;">
                            ...
                            if ($setting->destpp == 0) { ?>
                                ...
                                // $categxx = $db->query("SELECT * FROM products WHERE statuss=0 ORDER BY  name ASC ")->getResult();
                                $categxx = $ProductModel->where('statuss', 0)->orderBy('name', 'ASC')->findAll(50);
                                /*$categxx=mysql_query("CALL getproductsdata()");*/

                                foreach ($categxx as $product) {
                                    $cheked = true;
                                    $invis = $product->h_stores;
                                    $invis = !empty($invis) ? $product->h_stores : '';
                                    // $invis = !empty($invis) ?  trim($invis, ",") : [];
                                    $array = $invis != [] ? explode(',', $invis) : []; //split string into array seperated by ', '
                                    foreach ($array as $value) //loop over values
                                    {
                                        $cheked = $value == session()->get('store') ? false : $cheked;
                                    }
                                    if ($cheked) {
                                        ...
                                        if ($setting->disc_all == 1) {
                                            if ($sellrr == 0) {
                                                ...
                                                $olloo_nn = $db->query(("select pp_pro_price from price_marterr where  pp_pro_id ='" . $product->id . "' and  pp_price_type ='" . $sellrr . "' "))->getRowArray();
                                                if (isset($olloo_nn['pp_pro_price']) && $olloo_nn['pp_pro_price']) {
                                            ...
                                            if ($sellrr == 0) {
                                                ...
                                                $new_price_store_org = $product->price - (intval($product->descountperr) * intval($product->price)) / 100;
                                                $mnmz = (intval($product->descountperr) * intval($new_price_store_org)) / 100;
                                            } else {
                                                $olloo_nn = $db->query("select pp_pro_price from price_marterr where  pp_pro_id ='" . $product->id . "' and  pp_price_type ='" . $sellrr . "' ")->getRowArray();
                                                if (isset($olloo_nn['pp_pro_price']) && $olloo_nn['pp_pro_price']) {
                                        ...
                                        $new_price_store = !$product->taxmethod || $product->taxmethod == '0' ? floatval($new_price_store_org) : floatval($new_price_store_org) * (1 + (isset($tg) ? $tg : 1) / 100);
                                    ...
                                    ?>
                                            ...
                                            <div style="display: none;">
                                                ...
                                                <div class="product flat-box">
                                                    ...
                                                    <input type="hidden" id="idname-<?= $product->id ?>" name="name"
                                                        value="<?= $product->name ?>" />
                                                    <input type="hidden" id="idprice-<?= $product->id ?>" name="price"
                                                        value="<?= $new_price_store ?>" />
                                                    <input type="hidden" id="category" name="category"
                                                        value="<?= $product->category ?>" />
                                                    <input type="hidden" id="dispp-<?= $product->id ?>" name="dispp"
                                                        value="<?= $product->descountperr ?>" />
                                                    <input type="hidden" id="disamt-<?= $product->id ?>" name="disamt"
                                                        value="<?= $mnmz ?>" />
                                        ...
                                        <?php
                                        } else if ($setting->destpp == 2) {

                                        ?>

                                            <div class="col-sm-3 col-xs-3" style="padding: 2px;">
                                                <button
                                                    style="width: 100%;border: 0px;padding-left: 0px;height: 140px;padding-right: 0px;background: transparent;border: 1px solid #d0d1d3;"
                                                    class="addPct" id="product-<?= $product->code ?>" <?php if (isset($product->batchwiseupdate) && $product->batchwiseupdate == 1) { ?>
                                                    onclick="education_fields_prod('<?= $product->id ?>')" <?php } else { ?>
                                                    onclick="add_posalenk('<?= $product->id ?>')" <?php } ?>>
                                                    <div class="product flat-box">
                                                        ...
                                                        <?php if ($product->photo) { ?><img
                                                                src="<?= base_url() ?>public/files/products/<?= $product->photo ?>"
                                                                alt="<?= $product->name ?>"><?php } else {
                                                                                            ?>
                                                            <img src="<?= base_url() ?>public/files/products/noimage.png"
                                                                alt="<?= $product->name ?>">
                                        ...
                                        ?>
                                            ...
                                            <div class="col-sm-12 " style="padding-right: 6px;padding-left: 1px;">
                                                <a href="#" class="addPct" id="product-<?= $product->code ?>"
                                                    <?php if (isset($product->batchwiseupdate) && $product->batchwiseupdate == 1) { ?> onclick="education_fields_prod('<?= $product->id ?>')"
                                                    <?php } else { ?> onclick="add_posalenk('<?= $product->id ?>')"
                                                    <?php } ?>>


                                                    <div
                                                        style="border: 1px solid #c8d1db; background: linear-gradient(to bottom, #ffffff 0%,#eeeeee 100%); width: 100%; border-radius: 0px; text-align: left; background-size: cover; margin-top: 2px; overflow: hidden;">
                                                        <h3 style="margin-top: 7px;font-size: 15px;font-weight: bold; color: #000; margin-left: 5px;"
                                                            id="proname"><?= $product->name ?>
                                                            <?php
                                                            if ($product->offer_id > 0) {
                                                            ?>
                                                                <span style="color:#358ee0;"><strike>
                                                                        <?= number_format((float) $pr_org_price, $setting->decimals, '.', '') ?></strike></span>
                                                            <?php } ?>
                                                            <span style="color:#ff0000;float: right;">
                                                                -Rs.<?= number_format((float) $pro_new_price, $setting->decimals, '.', '') ?></span>
                                                            <?php
                                                            if ($setting->expi == 1) {
                                                            ...
                                                            ?>
                                                                <span
                                                                    style="color:#489103;float: right;font-size: 10px;"><?= $expire_1m ?></span>
                                                            <?php } ?>
                    ...
                    <script type="text/javascript">
                        function exefunction(cc) {
                            ...
                            if (lfckv == true) {
                                ...
                                $('#RemiseValue').text(k2.toFixed(<?= $setting->decimals ?>));
                                $('#total').text(k3.toFixed(<?= $setting->decimals ?>));
                                $('#Paid').val(k3.toFixed(<?= $setting->decimals ?>));
                                $('#Paidd').val(k3.toFixed(<?= $setting->decimals ?>));
                                $('#TotalModal').text('<?= label('Total') ?>Rs.' + k3.toFixed(<?= $setting->decimals ?>));
                            } else {
                                var bb = $('#subtot-' + cc).text();
                                var dsper = $('.Remise').val();
                        ...
                        $(document).ready(function() {
                            ...
                            $('.holdList').load("<?php echo site_url('pos/holdList/' . session()->get('register')); ?>");
                        ...
                        function total_change() {
                            ...
                            if (($('.TAX').val().indexOf('%') == -1) && ($('.Remise').val().indexOf('%') != -1)) {
                                ...
                                $('#taxValue').text(percentage($('#Subtot').text(), $('.TAX').val()).toFixed(
                                    <?= $setting->decimals ?>) + ' <?= $setting->currency ?>');
                                $('#RemiseValue').text('<?= $setting->currency ?>');
                                tot = tot - parseFloat($('.Remise').val() ? $('.Remise').val() : 0);
                                $('#total').text(tot.toFixed(<?= $setting->decimals ?>));
                                ...
                                $('#RemiseValue').text(percentage($('#Subtot').text(), $('.Remise').val()).toFixed(
                                    <?= $setting->decimals ?>));
                                $('#total').text(tot.toFixed(<?= $setting->decimals ?>));
                                $('#Paid').val(tot.toFixed(<?= $setting->decimals ?>));
                                $('#Paidd').val(tot.toFixed(<?= $setting->decimals ?>));
                                ...
                                $('#bamacee span').text("-" + tot.toFixed(<?= $setting->decimals ?>));
                                $('#disamtss').val(tyxz.toFixed(<?= $setting->decimals ?>));
                        ...
                        function total_changeddi() {
                            ...
                            if (($('.TAX').val().indexOf('%') == -1) && ($('.Remise').val().indexOf('%') != -1)) {
                                ...
                                $('#RemiseValue').text(dtot.toFixed(<?= $setting->decimals ?>));
                                $('#total').text(tot.toFixed(<?= $setting->decimals ?>));
                                $('#Paid').val(tot.toFixed(<?= $setting->decimals ?>));
                                $('#Paidd').val(tot.toFixed(<?= $setting->decimals ?>));
                                $('#TotalModal').text('<?= label('Total') ?> Rs.' + tot.toFixed(<?= $setting->decimals ?>));
                        ...
                        function AddHold() {
                            swal({
                                    ...
                                    text: 'Do you want to hold the customer?',
                                    type: "warning",
                                    showCancelButton: true,
                                    confirmButtonColor: "#DD6B55",
                                    confirmButtonText: 'Yes, Hold Customer',
                        ...
                        function RemoveHold() {
                            ...
                            var id = $('#customerSelect').find('option:selected').val();
                            var num_id = $('.selectedHold').attr('id');
                                    ...
                                    function() {
                                        ...
                                        $.ajax({
                                            ...
                                            success: function(data) {
                                                $.ajax({
                                                    type: "post",
                                                    url: "<?php echo site_url('pos/load_posales/'); ?>" + id + "?product_id=" + id,
                                                    data: {
                                                        product_id: id
                                                    },
                                                    dataType: "html",
                        ...
                        function add_posale(id) {
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    if (values[0] === 'stock') {
                                        $('#loadingimg').hide();
                                        swal("Available Quantity: " + values[3]);
                                        $('#countryname_1m').focus();
                                    } else {
                                        $.ajax({
                                            type: "post",
                                            url: "<?php echo site_url('pos/load_posales/'); ?>" + idkm + "?product_id=" + id,
                                            data: {
                                                product_id: id
                                            },
                                            dataType: "html",
                                        ...
                                        $('#ItemsNum span, #ItemsNum2 span').text(values[1]);

                                        $('#disamtt span').text(0);

                                        $('#Subtot').text(values[2]);
                        ...
                        // function add_posale(id) {
                        //     let name1 = $('#idname-' + id).val();
                        //     let price1 = parseFloat($('#idprice-' + id).val()) || 0;
                        //     let ddpp = parseFloat($('#dispp-' + id).val()) || 0;
                        //     let ddam = parseFloat($('#disamt-' + id).val()) || 0;
                        //     let idkm = $('#customerSelect').val();
                        //     let number = $('.selectedHold').text();
                        ...
                        //                 swal("Available Quantity: " + response.available_stock);
                        //                 $('#countryname_1m').focus();
                        //             } else {
                        //                 $('#productList').load("<?= site_url('pos/load_posales'); ?>/" + idkm);
                        //                 $('#ItemsNum span, #ItemsNum2 span').text(response.items_count);
                        //                 $('#disamtt span').text(0);
                        //                 $('#Subtot').text(response.subtotal);
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    if (values[0] === 'stock') {
                                        ...
                                        $.ajax({
                                            type: "post",
                                            url: "<?php echo site_url('pos/load_posales?product_id='); ?>" + id,
                                            data: {
                                                product_id: id
                                            },
                                            dataType: "html",
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    if (values[0] === 'stock') {
                                        ...
                                        $('#countryname_1m').val('');
                                        // $('#productList').html("<?php //echo site_url('pos/load_posales'); 
                                                                    ?>/" + idkm + "?product_id=" + id);
                        ...
                        function disxn() {
                            ...
                            $('#Paid').val(hjk.toFixed(<?= $setting->decimals ?>));
                            ...
                            var zs2 = $('#Paid').val();
                            var zs3 = parseFloat(zs1) - parseFloat(zs2) + parseFloat(tot_creaditpoint) + parseFloat(retamt) + parseFloat(
                                recivedamt2);

                            $('#bamacee span').text(zs3.toFixed(<?= $setting->decimals ?>));

                            document.getElementById("TotalModal").innerHTML = hjk.toFixed(2);
                            $('#Paidd').val(hjk.toFixed(<?= $setting->decimals ?>));
                        ...
                        function edit_posale(id) {
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    if (values[0] === 'stock') {
                                        swal("Available Quantity: " + values[1]);
                                        $('#productList').load("<?php echo site_url('pos/load_posales'); ?>/" + idkm);
                                        // $('.barcode').focus();
                                        // $('.barcode').val('');
                        ...
                        $("#customerSelect").change(function() {
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    $('#Paid').val(jjjj.toFixed(<?= $setting->decimals ?>));
                                    $('#Paidd').val(jjjj.toFixed(<?= $setting->decimals ?>));
                                    $('#TotalModal').text('<?= label('Total') ?>Rs.' + jjjj.toFixed(
                                        <?= $setting->decimals ?>));
                        ...
                        function cancelPOS() {
                            swal({
                                    ...
                                    showCancelButton: true,
                                    confirmButtonColor: "#DD6B55",
                                    confirmButtonText: '<?= label('Yes, delete it!') ?>',
                                    closeOnConfirm: false
                                },
                                function() {
                        ...
                        function saleBtn(type) {
                            ...
                            if (paidMethod == 2) {
                                paidMethod += '~' + $('#CreditCardNum').val() + '~' + $('#CreditCardHold').val() + '~' + $('#ChequeNum')
                                    .val();
                            } else {
                        ...
                        function total_noots() {
                            ...
                            var i;
                            for (i = 1; i < 9; i++) {
                                console.log(Number($('#saa_' + i).val()));

                                jj = Number($('#caa_' + i).val()) * Number($('#saa_' + i).val());
                                Number($('#kaa_' + i).val(jj));
                                mm = Number(mm) + Number(jj);
                            }
                            ll = Number(mm) - Number($('#expectedcash_1').val());
                            kk = Number($('#total_cl').val()) + Number(ll);


                            ($('#subtott').val(Number(mm)));
                            Number($('#countedcash_1').val(Number(mm)));
                            Number($('#diffcash_1').val(Number(ll)));
                            Number($('#countedtotal').val(Number(kk)));

                            fff = Number(kk) - Number($('#total_cl').val());

                            ($('#difftotal').val(Number(fff)));
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    $('#countedcash').on('keyup', function() {
                                        var change = -(parseFloat($('#expectedcash').text()) - parseFloat($(this)
                                            .val()));
                                        var difftot = change + parseFloat($('#diffcc').text()) + parseFloat($(
                                            '#diffcheque').text());
                                        var total = parseFloat($('#countedcc').val()) + parseFloat($('#countedcheque')
                                            .val()) + parseFloat($('#countedcash').val());
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                            aria-hidden="true">&times;</span></button>
                                    <h3 style="width:33%;float: left;font-family: bold;margin-top: 0px;margin-bottom: 1px;"
                                        id="TotalModal">
                                        </h4>
                                        <h4 style="width:25%;float: right;" class="modal-title" id="ItemsNum2">
                                            <?= label('item') ?> : <span></span> </h4>
                                </div>
                                <form>
                                    <div class="modal-body">

                                        <table class="table table-striped  "
                                            style="border: 1px solid #ddd;margin-bottom: 0px;font-family: Roboto,sans-serif;">



                                            <h5 style="display: none;" id="customerName">
                                                <span><?= label('WalkinCustomer') ?></span>
                                            </h5>



                                            <input value="" style="width: 80%;border: 1px solid #ccc;padding: 5px 12px;"
                                                type="hidden" name="custrrf" class="-control" id="custrrf" />
                                            ...
                                            if ($kkar['salretv'] == 1) { ?>
                                                ...
                                                <tr>
                                                    <td class="exchange"><?= label('Exchange') ?></td>
                                                    <td><?= label('Time') ?>(HH:MM)</td>
                                                ...
                                                <tr>
                                                    <td style="border-top: 0px solid #ddd;">
                                                        <input autocomplete="off"
                                                            style="padding: 5px 12px;width:100px;float: left;" type="text"
                                                            name="sals_iidr" class="form-control" id="sals_iidr"
                                                            placeholder="<?= label('Sales No') ?>">
                                                        <br>
                                                    ...
                                                    <td style="border-top: 0px solid #ddd;">
                                                        <input autocomplete="off"
                                                            style="padding: 5px 12px;width:100px;float: left;" type="text"
                                                            name="sals_time" class="form-control" id="sals_time">
                                                        <br>
                                                        <button
                                                            style="padding: 6px 0px;width:50px;float: left;margin-bottom: 0px;margin-left: 10px;margin-top: 5px;background-color: #489103;"
                                                            class="btn btn-add" type="button"
                                                            onclick="education_fields();">Check</button>
                                                    </td>



                                                    <td style="border-top: 0px solid #ddd;">
                                                        <input style="padding: 5px 12px;width:100px;float: left;" type="text"
                                                            name="sals_iidrproo" class="form-control" id="sals_iidrproo"
                                                            placeholder="<?= label('barcode') ?>">
                                                        <button
                                                            style="padding: 6px 0px;width:50px;float: left;margin-bottom: 0px;background-color: #489103;"
                                                            class="btn btn-add" type="button"
                                                            onclick="education_fieldsproductsold();">Check</button>
                                                    </td>


                                                    <td style="border-top: 0px solid #ddd;"><input readonly="readonly"
                                                            value="0"
                                                            style="border: 1px solid #ccc;padding: 5px 12px;width:80%;background-color: #edf2f6;"
                                                            type="text" name="retidd" class="-control" id="retidd" />
                                                    </td>

                                                    <td style="border-top: 0px solid #ddd;"><input value="0"
                                                            style="width:80%;border: 1px solid #ccc;padding: 5px 12px;background-color: #edf2f6;"
                                                            type="text" readonly="readonly" value="0" name="amttt"
                                                            class="-control" id="amttt" />
                                                    </td>
                                                </tr>
                                            ...
                                            <input readonly="readonly" type="hidden" value="0" name="paid"
                                                class="form-control <?= strval($setting->keyboard) === '1' ? 'paidk' : '' ?>"
                                                id="Paid" placeholder="<?= label('Paid') ?>">

                                            <input type="hidden" value="0" name="paidd"
                                                class="form-control <?= strval($setting->keyboard) === '1' ? 'paidk' : '' ?>"
                                                id="Paidd" placeholder="<?= label('Paid') ?>">
                                            ...
                                            <?php if ($setting->creditstatus == 1) { ?>
                                                ...
                                                <input type="hidden" style="width:57%;border: 1px solid #ccc;padding: 5px 12px;"
                                                    name="creddate"
                                                    class="form-control <?= strval($setting->keyboard) === '1' ? 'paidk' : '' ?>"
                                                    id="creddate" placeholder="<?= label('Credit Days') ?>">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <button type="button" class="btn btn-add"
                                            onclick="saleBtn(1)"><?= label('save&print') ?></button>
                                    </div>
                                    <?php echo form_close(); ?>
                            </div>
                        </div>
                                        ...
                                        $("#add_item").autocomplete({
                                            ...
                                            response: function(event, ui) {
                                                if ($(this).val().length >= 16 && ui.content[0].id == 0) {
                                                    ...
                                                    $('#add_item').focus();
                                                    $(this).val('');
                                                } else if (ui.content.length == 1 && ui.content[0].id !=
                                                    0) {
                    ...
                    <div class="modal fade" id="Addproduct" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body" style="padding: 2px;height: 400px; overflow: scroll;">

                                    <input type="hidden" readonly="readonly" maxlength="30" value="0" name="type"
                                        class="form-control" id="type" placeholder="<?= label('ProductCode') ?>">

                                    <div class="form-group controls">
                                        <div class="col-xs-4">
                                            <label for=""><?= label('HSN') ?></label>
                                            <input type="text" maxlength="30" value="" Required name="hsn"
                                                class="form-control" id="hsn" placeholder="<?= label('HSN') ?>">
                                        ...
                                        <div class="col-xs-4">
                                            <label for="Category">Brand</label><a href="javascript:void(0)"
                                                data-toggle="modal" data-target="#Addbrand">
                                                <span class=" fa-lg" data-toggle="tooltip" data-placement="top"
                                                    title="" data-original-title="Add New Brand">

                                                    <i style="color: #89b03e;padding: 5px;"
                                    ...
                                    <div class="form-group">
                                        <div class="col-xs-4">
                                            ...
                                            <a href="javascript:void(0)" data-toggle="modal" data-target="#Addcategory">
                                                <span class=" fa-lg" data-toggle="tooltip" data-placement="top"
                                                    title="" data-original-title="Add New Category">
                                        ...
                                        <div class="col-xs-4" id="supply">
                                            ...
                                            <a href="javascript:void(0)" data-toggle="modal" data-target="#AddSupplier">
                                                <span class="fa-lg" data-toggle="tooltip" data-placement="top"
                                                    title="" data-original-title="Add New Suppliers">

                                                    <i style="color: #89b03e;padding: 5px;"
                                                        class="fa fa-user-plus fa-stack-1x  "></i>
                                                </span>
                                            </a>
                                    ...
                                    <div class="form-group">
                                        <div class="col-xs-4">
                                            <label for="PurchasePrice"><?= label('PurchasePrice') ?>
                                                (<?= $setting->currency ?>)</label>
                                            <input type="number" Required name="cost" value="0"
                                                class="form-control" id="PurchasePrice"
                                                placeholder="<?= label('PurchasePrice') ?>">
                                        </div>

                                        <div class="col-xs-4">
                                            <label for="Price"><?= label('Selling') ?> <?= label('Price') ?></label>
                                            <input type="number" value="0" Required name="price"
                                                class="form-control" id="Price" placeholder="<?= label('Price') ?>">
                                        </div>

                                        <div class="col-xs-4">
                                            <label for="Price"><?= label('MRP') ?></label>
                                            <input type="number" value="0" Required name="rrate"
                                                class="form-control" id="rrate" placeholder="<?= label('MRP') ?>">
                                        </div>
                                    ...
                                    <?php
                                    $mkzz = $db->query("select * from settings where id=1 ")->getRowArray();
                                    if ($setting->gst_tax == 1) {
                                    ?>
                                        <div class="form-group">
                                            ...
                                            <div class="col-xs-12">
                                                ...
                                                <a href="javascript:void(0)" data-toggle="modal" data-target="#Addtax">
                                                    <span class="fa-lg" data-toggle="tooltip" data-placement="top"
                                                        title="" data-original-title="Add New Tax">

                                                        <i style="color: #89b03e;padding: 5px;text-align: left; margin-left: 14%;" class="fa fa-plus fa-stack-1x  "></i>
                                                    </span>
                                                </a>
                                                        ...
                                                        <div class="col-xs-4">
                                                            <span style="float: left;width: 10%">
                                                                <input type="checkbox" style="display: block;"
                                                                    name="ckk[]" id="ckc" value="<?php echo $taxxf['id']; ?>">
                                                            </span>
                                            ...
                                            <input type="hidden" value="0" maxlength="10" name="stax"
                                                class="form-control" id="sTax" placeholder="In %">

                                            <input type="hidden" value="0" maxlength="2" name="igst"
                                                class="form-control" id="igst" placeholder="In %">
                                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-xs-3">
                                            ...
                                            <input value="0" maxlength="2" type="number" step="any"
                                                name="dispx" class="form-control" id="dispx"
                                        ...
                                        <div class="col-xs-3">
                                            ...
                                            <input Required type="text" name="net_wight" value="0"
                                                class="form-control" id="net_wight" />
                                        </div>

                                        <div class="col-xs-3">
                                            <label for="Unit"><?= label('Unit') ?></label>
                                            <input value="0" Required type="text" step="any" name="unit"
                                                class="form-control" id="Unit" placeholder="<?= label('Unit') ?>">
                                        </div>

                                        <div class="col-xs-3">
                                            <label for="AlertQt"><?= label('AlertQt') ?></label>
                                            <input type="number" value="0" name="alertqt" class="form-control"
                                                id="AlertQt" placeholder="<?= label('AlertQt') ?>">
                                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-xs-3">
                                            ...
                                            <input type="text" value="0" Required name="packed_1m"
                                                class="form-control" id="packed_1m">
                                        </div>
                                        ...
                                        <div class="col-xs-3">
                                            ...
                                            <input type="text" value="0" name="best_before" class="form-control"
                                                id="best_before">

                                        </div>


                                        <input value="1" type="hidden" name="measur" class="form-control"
                                            id="measur">
                                    ...
                                    <div class="form-group">
                                        <div class="col-xs-12">
                                            ...
                                            <input type="text" value="<?php echo @$mmik['id'] + 1; ?>" name="code"
                                                class="form-control" id="ProductCode"
                                                placeholder="<?= label('ProductCode') ?>">
                                    ...
                                    <input type="hidden" name="color" id="option7" value="color07"
                                        autocomplete="off">


                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-default"
                                            data-dismiss="modal"><?= label('Close') ?></button>
                                        <button type="submit" class="btn btn-add"><?= label('Submit') ?></button>
                                    </div>
                                    <?php echo form_close(); ?>
                                </div>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-default hiddenpr"
                                onclick="location.reload();"><?= label('Close') ?></button>
                            <button type="button" class="btn btn-add hiddenpr"
                                onclick="addcombo()"><?= label('submit') ?></button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div style="width:1px;height: 1px;" id="ticket">
                <div style="width:1px;height: 1px;">
                    <div class="modal-content" style="width:1px;height: 1px;">
                        <div class="modal-header" style="width:1px;height: 1px;">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                    aria-hidden="true">&times;</span></button>
                            <h4 class="modal-title" id="ticket"><?= label('Receipt') ?></h4>
                        </div>
                        <div class="modal-body" id="modal-body" style="width:1px;height: 1px;">
                            <div id="printSection">
                                <!-- Ticket goes here -->
                                <center>
            ...
            <div class="modal fade" id="education_fields" tabindex="-1" role="dialog"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-xs-6">
                                    ...
                                    <input type="text" name="name" maxlength="50" Required
                                        ...
                                        placeholder="<?= label('CustomerName') ?>">
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <div class="col-xs-6">
                                    ...
                                    <input type="text" name="phone" maxlength="30" class="form-control"
                                        id="CustomerPhone" placeholder="<?= label('CustomerPhone') ?>">
                                </div>
                                <div class="col-xs-6">
                                    <label for="CustomerPhone"><?= label('GST') ?></label>
                                    <input type="text" name="gstno" maxlength="30" class="form-control"
                                        id="gstno" placeholder="GST">
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <div class="col-xs-6">
                                    ...
                                    <input type="email" maxlength="50" name="email" class="form-control"
                                        id="CustomerEmail" placeholder="<?= label('CustomerEmail') ?>">
                                </div>
                                <div class="col-xs-6">
                                    <label for="CustomerDiscount"><?= label('CustomerDiscount') ?> (in %)</label>
                                    <input type="text" maxlength="2" name="discount" class="form-control"
                                        id="CustomerDiscount" placeholder="<?= label('CustomerDiscount') ?>">
                                </div>
                            </div>


                            <?php
                            if ($setting->birth_anni_modul == 1) {
                            ?> <div class="form-group">
                                    <div class="col-md-6">


                                        <label for="CustomerDiscount"><?= label('Birthday') ?></label>
                                        <input type="text" name="birthday_date" value="<?php echo date('d-m-Y'); ?>"
                                            class="form-control" id="birthday_date">
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" name="anniversary_date" class="form-control"
                                            value="<?php echo date('d-m-Y'); ?>" id="anniversary_date">
                                    </div>
                                </div>
                            <?php } else {
                            ...
                            ?>
                                ...
                                <input type="hidden" maxlength="30" value="<?php echo date('d-m-Y'); ?>" name="birthday_date"
                                    class="form-control" id="birthday_date" placeholder="Date">

                                <input type="hidden" maxlength="30" value="<?php echo date('d-m-Y'); ?>"
                                    name="anniversary_date" class="form-control" id="anniversary_date"
                                    placeholder="Date">
                            ...
                            <div class="form-group">
                                <div class="col-xs-6">
                                    ...
                                    <select class="form-control" name="custstate" id="custstate">
                                        ...
                                        <option <?php if ($setting->mystate == 1) { ?> selected="selected" <?php  } ?>
                                            value="1">ANDHRA PRADESH</option>
                                        <option value="3">ARUNACHAL PRADESH</option>
                                        <option value="2">ASSAM</option>
                                        <option <?php if ($setting->mystate == 5) { ?> selected="selected" <?php  } ?>
                                        ...
                                        <option <?php if ($setting->mystate == 4) { ?> selected="selected" <?php  } ?>
                                            value="4">GUJRAT</option>
                                        <option value="6">HARYANA</option>
                                        <option value="7">HIMACHAL PRADESH</option>
                                        <option value="8">JAMMU &amp; KASHMIR</option>
                                        <option value="34">JHARKHAND</option>
                                        <option <?php if ($setting->mystate == 9) { ?> selected="selected" <?php  } ?>
                                        ...
                                        <option <?php if ($setting->mystate == 21) { ?> selected="selected" <?php  } ?>
                            ...
                            <div class="col-xs-12">
                                ...
                                <input type="hidden" maxlength="3" name="creddate" class="form-control"
                                    id="creddate" placeholder="<?= label('Credit Days') ?>">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit"
                                style="background-color: #34495E;color:#fff;border: 1px solid transparent;padding: 6px 12px;float: right;margin: 10px 28px 0px 10px;"><?= label('Submit') ?></button>

                            <button type="button"
                                style="padding: 6px 12px;float: right;margin: 10px 5px 0px 5px;border: 1px solid #ccc;"
                                data-dismiss="modal"><?= label('Close') ?></button>
            ...
            <!-- Modal add user -->
            <div class="modal fade" id="CloseRegister" tabindex="-1" role="dialog"
                aria-labelledby="myModalLabel">
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body" style="height: 600px !important; overflow: auto;">
        ...
        <script type="text/javascript">
            function education_fields() {
                var totalItems = $('#ItemsNum span').text();
                var retunbil = $('#retunbill').val();
                if (totalItems == 0 && retunbil == 2) {
                    swal("Please add the product for exchange...");
                    $('#countryname_1m').focus();
                    return false;
                }
                var vl = $('#sals_iidr').val();
                var sals_time = $('#sals_time').val();
                var retunbill = $('#retunbill').val();

                if ((vl > 0 || v2 > 0) && sals_time != '') {

                    var datastring = 'countid=' + vl + '&retunbill=' + retunbill + '&sals_time=' + sals_time;
                    $.ajax({
                        type: "POST",
                    ...
                    alert("Please Enter Sales ID and Time ");
                    return false;
                }
            ...
            function gauthamm(ccc) {
                ...
                var ret_idd = document.getElementById('ret_idd').value;
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#bamacee span').text(dsds.toFixed(<?= $setting->decimals ?>));
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="text" maxlength="50" name="name" class="form-control"
                                id="CategoryName" placeholder="<?= label('CategoryName') ?>" required>
                        </div>
                    </div>
                    <div class="modal-footer">

                        <button data-dismiss="modal" onclick="return kakkakat();" type="submit"
                            style="background-color: #34495E;color:#fff;border: 1px solid transparent;padding: 6px 12px;float: right;margin: 10px 28px 0px 10px;"><?= label('Submit') ?></button>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="text" maxlength="50" name="Brandname" class="form-control"
                                id="Brandname" placeholder="<?= label('Brand') ?>" required>
                        </div>
                    </div>
                    <div class="modal-footer">

                        <button data-dismiss="modal" onclick="return kakkakbar();" type="submit"
                            style="background-color: #34495E;color:#fff;border: 1px solid transparent;padding: 6px 12px;float: right;margin: 10px 28px 0px 10px;"><?= label('Submit') ?></button>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="text" maxlength="50" name="taxName" class="form-control"
                                id="taxName" required>
                        </div>

                        <div class="form-group">
                            <label for="CategoryName"><?= label('tax') ?>(%)</label>
                            <input type="text" maxlength="2" name="persent" class="form-control"
                                id="persent" required>
                        </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button data-dismiss="modal" onclick="return kakkaktax();" type="submit"
                            style="background-color: #34495E;color:#fff;border: 1px solid transparent;padding: 6px 12px;float: right;margin: 10px 28px 0px 10px;"><?= label('Submit') ?></button>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-xs-6">
                                ...
                                <input type="email" maxlength="50" name="email" class="form-control"
                                    id="SupplierEmail" placeholder="<?= label('SupplierEmail') ?>">
                            </div>
                            <div class="form-group">
                                <div class="col-xs-6">
                                ...
                                <div class="col-xs-6">
                                    ...
                                    <input name="country" class="form-control" required="" id="country"
                            ...
                            <div class="col-xs-6">
                                ...
                                <input type="text" maxlength="50" name="gst" class="form-control"
                                    id="gst" placeholder="GST <?= label('Number') ?>">
                            </div>
                        ...
                        <input name="city" value="Chennai" class="form-control" id="city" Required
                            placeholder="City" type="hidden">





                        <input name="country" class="form-control" value="India" Required id="country"
                            placeholder="Country" type="hidden">
                    ...
                    <div class="modal-footer">
                        ...
                        <button data-dismiss="modal" onclick="return kakkak();" type="submit"
                            style="background-color: #34495E;color:#fff;border: 1px solid transparent;padding: 6px 12px;float: right;margin: 10px 28px 0px 10px;"><?= label('Submit') ?></button>
        ...
        <script type="text/javascript">
            function retunbill(vall) {
                $('#retunbill').val(vall);
                if (vall == 2) {
                    var exchange_html = "Exchange";
                } else {
                    var exchange_html = "Return";
                }
                $('.exchange').html(exchange_html);
            }
