...
if ($_POST['arrayidd']) {
    ...
    $width = 2;
    $widthxc = 0;
    $kk = isset($row) && $row != 0 ? (12 / $row) * ($printi['bar_width'] + 1) : 0;
    $content .= '<div>';
    $setting = $db->table('settings')->get()->getRow();

    for ($ij = 0; $ij < count($_POST['arrayidd']); $ij++) {
        ...
        $row = 2;
        $num = $_POST['arrayqtt'][$ij];
        $productBcode = $_POST['arrayidd'][$ij];


        $nnm = $db->query("select * from products where code='" . $productBcode . "' ")->getRowArray();
        $nam   = $nnm['name'];
        $dis_amt = $nnm['dis_amt'];
        $mrp = $nnm['rrate'];
        ...
        $tg = floatval($nnm['tax']) + floatval($nnm['sgst']);
        ...
        if (
            $nnm['dis_amt'] != null && $nnm['dis_amt'] > 0
        ) {
            $frr = '<span style="color:#358ee0;font-size: 10px;" >MRP: <strike  style="font-size: 13px;">' . number_format((float) $mrp, $setting->decimals, '.', '') . '</strike>
            ...
            for ($rf = 0; $rf < $row; $rf++) {
                ...
                if ($brcode_no == 1) {
                    ...
                    $content .= '<div   style="text-align: left;padding-right: 0px;padding-left: 0px;width:' . $printi['bar_width'] . 'mm;height:' . $printi['bar_hight'] . 'mm;float:left;"  >
                                ...
                                <tr>
                                <td colspan="2" style="width:25%;padding: 0px;text-align:Left;border: 0px solid #fff;background-color: white;"><img  style="margin-top:' . $printi['bar_img_top'] . 'px;width:' . $printi['bar_img_width'] . 'mm;height:' . $printi['bar_img_height'] . 'mm;font-size:' . $printi['bar_img_fontsize'] . 'px;" src="' . base_url() . 'public/assets/abc/qrimg' . $time_now . '.png" alt="' . $productBcode . '" />
                                </td>
                                <td colspan="4" style="width:72%;padding: 0px;text-align:left;border: 0px solid #fff;background-color: white;">' . $frr . '
