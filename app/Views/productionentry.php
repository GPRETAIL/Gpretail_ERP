...
?>
    ...
    <h3><?=label('Production');?> <?=label('Entry');?><?php if($kkar['prdena']==1){ ?><a style="float: right;" class="btn btn-primary btn-green" href="<?php echo base_url();?>production/add"  ><?=label('Add');?></a><?php } ?>
  ...
  var save_method; //for save method string
    ...
    $(document).ready(function() {
      table = $('#table').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('invoices_pur/ajax_list_production')?>",
            "type": "POST"
        },
    ...
    function showTicket(id){
     ...
     function showTicket_phycal(id){
      ...
      $.ajax({
          url : "<?php echo site_url('invoices_pur/ShowTicket_production')?>/"+id,
          type: "POST",
          success: function(data)
          {
