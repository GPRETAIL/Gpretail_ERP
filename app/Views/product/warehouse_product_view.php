...
<div class="container">
    ...
    <h3> <?= label("Warehouse Products"); ?>
        <?php if ($kkar['pra'] == 1) { ?>
            ...
            <div class=" float-right">
                <a class="btn btn-dark btn-green" href="<?= base_url('products/0?per_page=1000') ?>" style="margin-right: 5px;"><?= label("Show All"); ?></a>
            </div> -->
    ...
    <div class="row" style="margin-top:10px;">
        <form action="<?= base_url('products') ?>" method="POST">
            <div class="input-group" style="float: right;">
                ...
                <input type="text" class="form-control" placeholder="Enter Product Name or Barcode" value="<?= isset($_POST['productInput']) ? $_POST['productInput'] : '' ?>" name="productInput" id="productInput">
                <!-- <button type="submit" class="btn btn-primary" id="searchProduct" style="float: right; margin-top: 10px;">Submit</button> -->
            </div>
        </form>
        <table id="Table3s" class="table table-striped table-bordered" cellspacing="0" width="100%">
            <thead>
                <tr>
                    ...
                    <th class="hidden-xs">Pur ID</th>
                    <th><?= label("SupplierName"); ?></th>
                    <th><?= label("Product"); ?> <?= label("Name"); ?></th>
                    <th><?= label("Brand"); ?> <?= label("Name"); ?></th>

                    <?php
                    $mm = $db->query("select * from settings where id=1 ")->getRowArray();
                    if ($mm['gst_tax'] == 1) {
                    ?>
                    ...
                    <th><?= label("MRP"); ?></th>
                    <th><?= label("Dis"); ?> %</th>
                    <th><?= label("Store Avail"); ?> Qty</th>
                    <th><?= label("Warehouse Avail"); ?> Qty</th>
                    <th>Total</th>
            ...
            <tbody>
                <?php //$this->load->view('product/product_list_table', ['products' => $productList])
                ?>
            </tbody>
        ...
        <nav aria-label="Page navigation">
            <ul class="pagination">
                <?= isset($paginate) ? $paginate : '' ?>
            </ul>
        </nav>

        <link rel="stylesheet" type="text/css" href="<?php echo base_url(); ?>assets/new/datatables.min.css" />

        <script type="text/javascript" src="<?php echo base_url(); ?>assets/new/datatables.min.js"></script>
    </div>
    ...
    <script type="text/javascript">
        ...
        function loadTable() {
            ...
            $('#Table3s').DataTable({
                ...
                "pageLength": -1,
                "ajax": {
                    'url': '<?= site_url('InvoicesPro/warehouseProducts/' . (isset($offset) ? $offset : 0) . '/' . $per_page) ?>',
                    'type': 'post',
                    'data': {
                        user_id: 4,
    ...
    <script type="text/javascript">
        ...
        /*    $(document).ready(function() {
              table = $('#Table3s').DataTable({
                ...
                "ajax": {
                    "url": "<?php echo site_url('invoices_pro/warehouse_products') ?>",
                    "type": "POST"
                },
                    ...
                    <div class="form-group">
                        <div class="col-xs-4">
                            ...
                            <select class="form-control" name="brandd" id="brandd">
                                ...
                                <?php
                                $imnn = $db->query("SELECT * FROM brand ORDER BY name ASC")->getResult(); // ← Fix here
                                foreach ($imnn as $imnnf) {
                                ?>
                                    <option value="<?= esc($imnnf->id); ?>"><?= esc($imnnf->name); ?></option>
                                <?php
                                }
                                ?>
                            </select>
                                    ...
                                    ?>
                                        <div class="col-xs-4">
                                            <span style="float: left;width: 10%">
                                                <input type="checkbox" name="ckk[]" id="ckc" value="<?= esc($taxxf['id']); ?>" style="display: block;">
                                            </span>
                                            <span style="float: left;width:80%;margin-left:5%;">
                                                <?= esc($taxxf['name']); ?> - <?= esc($taxxf['valueper']); ?>%
                                            </span>
                                        </div>
                                    <?php
                                    }
