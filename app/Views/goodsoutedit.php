...
$perr=mysql_fetch_array(mysql_query("select * from goodsout where idd='".$segg."' "));
...
<div class="container">
   <div class="row" style="margin-top:10px;">
      <div class="col-md-12">
      <h3><?=label("Edit");?><a style="float: right;" class="btn btn-primary btn-red" href="<?php echo base_url();?>purchase"  ><?=label("Back");?> </a></h3>
<hr>
<?php 
...
<?php
@$oqpq=mysql_fetch_array(mysql_query("select * from goodsout order by idd desc "));
$lmll=@$oqpq['idd']|+1;
?>
<div class="col-sm-12">
         ...
         <div class="form-group"><?=label("Voucher");?> <?=label("Number");?>  
            <input readonly="readonly" type="text" value="<?php echo $perr['idd'];?>" name="autoidd" id="autoidd" class="form-control">
         </div>  
         </div>  

         <div class="col-sm-3">
         <div class="form-group"><?=label("WareHouse");?> 
       
        <select class=" form-control" id="warSelect" name="warSelect">
              
              <?php  $mkmf= mysql_fetch_array(mysql_query("select * from warehouses where id='".$perr['wareid']."'  ")) ;

              
              $jnn=$perr['dateof'];
              $rt=explode('-', $jnn);
              $jnngg=$rt[1].'/'.$rt[2].'/'.$rt[0];
         ...
         <div class="form-group"><?=label("Date");?> 
            <input type="text" name="pddate" class="form-control" id="pddate" value="<?php echo $jnngg;?>">
         ...
         <div class="col-sm-3">
           <div class="form-group"><?=label("Reference");?> <?=label("Number");?>
            <input type="text" name="refff" class="form-control" id="refff" value="<?php echo $perr['refno'];?>" >
...
<div class="col-sm-12">
<div class="col-sm-3">
           <div class="form-group"><?=label("Barcode");?> <?=label("Number");?>  
            <form onsubmit="return barcodekar()">
               <input style="margin-top:0px;" type="text" autofocus id="<?=strval($this->setting->keyboard) === '1' ? 'keyboard' : ''?>" class="form-control barcode" placeholder="<?=label('BarcodeScanner');?>">
            </form>
...
<div class="form-group"><?=label("Brand");?>  
      ...
      <div class="col-sm-3 "><div class="form-group"><?=label("Product");?> 
...
<div class="panel panel-default">
  ...
  <div class="panel-body">
         ...
         <div class="col-xs-2 table-header nopadding">
            <lable style="margin-top:10px;text-align: right;">Avail<?=label("Quantity");?></lable>
         </div>
...
<hr>
         ...
         <div class="footer-section">
            <div class="table-responsive col-sm-12 totalTab">
               <table class="table">
                  <tr>
                     <td style="margin-top:10px;text-align: right;padding: 4px;" class="" width="42%"><?=label("TotalItems");?></td>
                     <td style="padding: 4px;" class="whiteBg" width="58%">
                     <span class="float-left"><b id="ItemsNum"><span></span></b></span>
            ...
            <button style="margin-left: 25px;" type="button" class="btn btn-green col-md-2" data-toggle="modal" onclick=" kakkaka();"  ><?=label('PAYEMENT');?></button>
         </div>
...
var reff=$('#refff').val();
   ...
   $.ajax({
      ...
      type: "POST",
      data: {warr: warr, reff: reff, ppd: ppd, qtt: qtt, idd: idd,cccc:cccc},
      success: function(data)
      {
...
$('#productListkare').load("<?php echo site_url('pos/load_pogoodsed/'.$segg)?>");
...
var mmn=<?=$this->register;?>;
   ...
   var number = $('.selectedHold').clone().children().remove().end().text();
     ...
     $.ajax({
         ...
         type: "POST",
         data: {name: name1, price: price1, product_id: id, number: number, registerid: <?=$this->register;?>,warsel:warsel},
         success: function(data)
         {
...
function saleBtn(type) {
  ...
  $.ajax({
      ...
      type: "POST",
      data: {recivamt:recivamt,ballamtt:ballamtt,client_id: clientID, clientname: clientName, discountamount: discountamount,  tax: Tax, discount: Discount, subtotal: Subtotal, total: Total, created_by: createdBy, totalitems: totalItems, paid: Paid, status: Status, paidmethod: paidMethod, ccnum: ccnum, ccmonth: ccmonth, ccyear: ccyear, ccv: ccv,taxamount:cgggst,sgsttaxamt:sgggst,lalid:lalid,lalamt:lalamt,discount_indujul : xsxsx},
      success: function(data)
      {
...
else
{
 ...
 $('#bamacee span').text(bb.toFixed(<?=$this->setting->decimals;?>));
...
<script type="text/javascript">
   ...
   $('#bamacee span').text(kmxx.toFixed(<?=$this->setting->decimals;?>));
           ...
           <div class="form-group Paid">
             <label for="Paid"><?=label("Amount");?>  <?=label("To");?>  <?=label("Pay");?>   </label>

             
             <input readonly="readonly" type="text" value="0" name="paid" class="form-control <?=strval($this->setting->keyboard) === '1' ? 'paidk' : ''?>" id="Paid" placeholder="<?=label("Paid");?>">
           ...
           <div class="form-group Paid">
             <label for="Paid"><?=label("Received");?>  <?=label("Amount");?>  </label>


             <input type="text" value="0" onkeyup="return callcc(this.value,this.id);" name="recivedamt" class="form-control <?=strval($this->setting->keyboard) === '1' ? 'paidk' : ''?>" id="recivedamt" >
          ...
          <div class="form-group ReturnChange">
             <h3 id="bamacee" ><?=label("Balance");?>  <?=label("Rs");?>.<span></span> </h3>
          </div>
