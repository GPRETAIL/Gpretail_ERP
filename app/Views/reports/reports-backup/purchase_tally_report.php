<table id="chltkarr" class="table table-striped table-bordered" cellspacing="0" width="100%">
    <thead>
        <tr class="hideme">
            <th colspan="44" style="text-align:center;"><?= esc($company) ?></th>
        </tr>
        <tr class="hideme" style="text-align:center;">
            <th colspan="44"><?= esc($address) ?></th>
        </tr>
        <tr class="hideme" style="text-align:center;">
            <th colspan="44">Purchase Tally Report from <?= esc($start) ?> Till <?= esc($end) ?></th>
        </tr>
    ...
    <tbody>
        ...
        foreach ($items as $prd):
            $supplier = $db->table('suppliers')->where('id', $prd->supplier)->get()->getRowArray();
            $purchase = $db->table('purchases')->where('id', $prd->purchase_id)->get()->getRowArray();
        ...
        <tr>
            ...
            <td><?= esc($prd->purchase_id) ?></td>
            <td><?= esc($prd->supplier) ?></td>
            <td><?= date("d/m/Y", strtotime($purchase['invdat'] ?? $prd->ndate)) ?></td>
            <td>Purchase</td>
