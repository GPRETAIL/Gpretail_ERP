<table id="chltkarr" class="table table-striped table-bordered" width="100%">
    <thead>
        <tr class="hideme"><th colspan="<?= 3 + count($taxRates) ?>" class="text-center"><?= esc($settings['companyname']) ?></th></tr>
        <tr class="hideme text-center"><th colspan="<?= 3 + count($taxRates) ?>"><?= esc($store['adresse'] ?? '') ?></th></tr>
        <tr class="hideme text-center"><th colspan="<?= 3 + count($taxRates) ?>">Customer Tax Reports from <?= esc($start) ?> to <?= esc($end) ?></th></tr>
