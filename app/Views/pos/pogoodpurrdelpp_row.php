<div id="add<?= $count ?>" class="col-xs-12">
    <div class="col-sm-3" style="padding-right: 1px; padding-left: 1px;">
        <div class="form-group">
            ...
            <input type="hidden" class="form-control" value="<?= esc($item->producnum) ?>" id="statediv_<?= $count ?>" name="statediv[]" />
            <input type="hidden" class="form-control" value="<?= esc($item->ppitemid) ?>" id="peritemid_<?= $count ?>" name="peritemid[]" />
    ...
    <?php else: ?>
        <input readonly type="hidden" class="form-control" id="cgst_<?= $count ?>" name="cgst[]" value="<?= esc($item->cgstt) ?>">
        <input readonly type="hidden" class="form-control" id="sgst_<?= $count ?>" name="sgst[]" value="<?= esc($item->sgst) ?>">
    <?php endif; ?>
