<div class="container">
    ...
    if ($setting->combo == 0 || $kkar['comv'] == 0) {
        redirect('help');
    }
    ...
    <h3><?= label('combooffers'); ?> <?php if (isset($kkar['coma']) && $kkar['coma'] == 1) { ?><a style="float: right;" class="btn btn-primary btn-green" href="<?php echo base_url('Combooffer/add'); ?>"><?= label('Add Combo Offer'); ?></a>
        <?php } ?>
    ...
    <table id="Table" class="table table-striped table-bordered" cellspacing="0" width="100%">
        <thead class="thead-inverse">
            <tr>
                ...
                <th style="text-align: center;"><?= label('Offer Title'); ?></th>
                ...
                <th style="text-align: center;"><?= label('Total selling price'); ?></th>
                <th style="text-align: center;"><?= label('Offer'); ?> <?= label('No Of Items'); ?> </th>
        ...
        <tbody>
            ...
            $rolr = $user->role;
            $kkar = $db->query(("SELECT * FROM permission_new WHERE nname='" . $rolr . "'  "))->getRowArray();
            $query = $db->query("SELECT *  FROM purchases_combo   ORDER BY id DESC ");
            ...
            foreach ($list as $invoice) {
                ...
                $lk_tot = $db->query(("select sum(qt) as ddc from purchase_items_combo where purchase_id='" . $invoice->id . "' "))->getRowArray();
            ?>
                <tr role="row" class="odd">
                    <td style="text-align: center;"><?php echo date("d-m-Y", strtotime($invoice->purdat)); ?></td>
                    <td style="text-align: center;"><?php echo sprintf("%05d", $invoice->id); ?></td>
                    <td style="text-align: center;"><?php echo  $invoice->ref; ?></td>
                    <td style="text-align: right;"><?php echo $invoice->total; ?></td>
                    <td style="text-align: right;"> <?php echo $lk_tot['ddc']; ?></td>
                    <td style="text-align: center;"><?= $invoice->betot ?></td>
                    <td style="text-align: center;"><?= $invoice->valid_from ?></td>
                    <td style="text-align: center;"><?= $invoice->valid_to ?></td>
                    <td style="text-align: center;"> <?php echo $invoice->created_by; ?> </td>
                    ...
                    <td style="text-align: center;">
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu">
                                <li><a href="javascript:void(0)" onclick="showTicket(<?php echo $invoice->id; ?>, '<?= $invoice->store_id ?>')"><i class="fa fa-ticket fa-fw" aria-hidden="true"></i><?php echo  label("View"); ?></a></li>
                                <?php
                                if ($kkar['comd'] == 1) {
                                ?>
...
<script type="text/javascript">
    ...
    function delete_invoice(id) {
        swal({
            ...
            function() {
                ...
                $.ajax({
                    url: "<?php echo site_url('InvoicesPur/ajax_delete_combo') ?>/" + id,
                    type: "POST",
                    dataType: "JSON",
    ...
    function showTicket(id, store_id = 0) {

        $.ajax({
            url: "<?php echo site_url('InvoicesPur/ShowTicket_combo') ?>/" + id,
            data: {
                storer: store_id
            },
            type: "POST",
    ...
    function pdfreceipt() {
        ...
        redirectPost('<?php echo site_url('pos/pdfreceipt') ?>', {
            'content': content
        });
        // $.redirect('<?php echo site_url('pos/pdfreceipt') ?>?content=' + content, {
        //     content: content
        // });
        // $.post('<?php echo site_url('pos/pdfreceipt') ?>', {
        //     content: content
        // });
                ...
                <div class="modal-body" id="modal-body">
                    <div id="printSection">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-xs-3 " style="margin-left:20px;">
                                    ...
                                    <select style="width:150px;float: left;" required="required" class="form-control" id="warr" name="warr">
                                        ...
                                        <?php
                                        $omaa = $db->query("select * from warehouses order by name asc ")->getResultArray();
                                        foreach ($omaa as $omaaf) { ?>
                                ...
                                <div class="col-xs-3 " style="margin-left:20px;">
                                    ...
                                    <select style="width:150px;float: left;" onchange="getState(this.value,this.id)" class="form-control" id="brnadd" name="brnadd">
                                        <option value="">Select</option>
                                        <?php
                                ...
                                <div class="col-xs-3 " style="margin-left:20px;">
                                    ...
                                    <select required="required" style="width:150px;float: left;" class=" form-control" id="statediv" name="statediv" onchange="getdetals(this.value,this.id)">
                                        ...
                                        <?php
                                        $omaapp = $db->query("SELECT * FROM products ORDER BY name ASC LIMIT 100 ")->getResultArray();
                                        foreach ($omaapp as $omaappf) { ?>

                                            <option value="<?php echo $omaappf['id']; ?>"><?php echo $omaappf['name']; ?></option>
                        ...
                        <div class="col-xs-12 " style="margin-top:10px;">
                            ...
                            <?php
                            $mmm = $db->query("SELECT * FROM stores ORDER BY name ASC LIMIT 100 ")->getResultArray();
                            foreach ($mmm as $mmmf) {
                            ?>
                                <div class="form-group">
                                    <div class="col-xs-6 ">
