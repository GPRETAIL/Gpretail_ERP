...
class TCPDF
{
	...
	public function getRawCharWidth($char)
	{
		...
		// if ((int) (log10($this->CurrentFont['cw'][$char]))) {
		// 	$w = $this->CurrentFont['cw'][$char];
		// } elseif ((int) (log10($this->CurrentFont['dw']))) {
		// 	// default width
		// 	$w = $this->CurrentFont['dw'];
		// } elseif ((int) (log10($this->CurrentFont['cw'][32]))) {
		// 	// default width
		...
		while ($elkey < $maxel) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					// foreach (list($id, $name) = each($attr_array[1])) {
