...
class Reports extends BaseController
{
  ...
  <tr>
    ...
    public function getSalesReportDetailed()
    {
        ...
        // (see SalesReportService::buildDetailedQuery() note) so they're
        // not sortable; fall back to date ordering if requested.
        $columns = [
        ...
        $recordsTotal = $svc->buildDetailedQuery(array_merge($f, ['search' => '']))->countAllResults(false);

        $builder = $svc->buildDetailedQuery($f, $colFilters);
        $recordsFiltered = $builder->countAllResults(false);

        $rows = $svc->buildDetailedQuery($f, $colFilters)
            ->orderBy($orderBy, $orderDir)
            ->limit($length, $start)
            ->get()
            ->getResultArray();
        $rows = $svc->attachTaxAndReturns($rows);
    ...
    public function getSalesReportSummary()
    {
        ...
        $recordsTotal = $svc->buildSummaryQuery(array_merge($f, ['search' => '']), $group)->countAllResults(false);
        $recordsFiltered = $svc->buildSummaryQuery($f, $group, $colFilters)->countAllResults(false);

        $rows = $svc->buildSummaryQuery($f, $group, $colFilters)
            ->orderBy('period', 'desc')
            ->limit($length, $start)
            ->get()
            ->getResultArray();
        $rows = $svc->attachPeriodTaxAndReturns($rows, $group, $f);
    ...
    public function exportSalesReport()
    {
        ...
        if ($mode === 'detailed') {
            ...
            $baseBuilderFn = fn() => $svc->buildDetailedQuery($f)->orderBy('s.created_at', 'asc');
            $fetchBatch = fn(int $batch, int $offset) => $svc->attachTaxAndReturns($baseBuilderFn()->limit($batch, $offset)->get()->getResultArray());
        } else {
            ...
            $baseBuilderFn = fn() => $svc->buildSummaryQuery($f, $group)->orderBy('period', 'asc');
            $fetchBatch = fn(int $batch, int $offset) => $svc->attachPeriodTaxAndReturns($baseBuilderFn()->limit($batch, $offset)->get()->getResultArray(), $group, $f);
        }
