...
class InvoiceOcrService
{
    ...
    private function parseInvoice(string $text): array
    {
        ...
        return [
            ...
            'invoice' => ['number' => $this->match($text, '/(?:invoice|inv|bill)\s*(?:no|number|#)?\s*[:\-]?\s*([A-Z0-9][A-Z0-9.\/_-]{2,})/i', 1), 'date' => $this->match($text, '/\b(?:\d{1,2}[-\/.]\d{1,2}[-\/.]\d{2,4}|\d{4}[-\/.]\d{1,2}[-\/.]\d{1,2})\b/')],
    ...
    {
        foreach (array_slice($lines, 0, 10) as $line) if ($line !== '' && strlen($line) >= 3 && !preg_match('/invoice|tax invoice|gstin|gst no|bill no|date/i', $line)) return $line;
        return null;
    }
    ...
    {
        foreach ($lines as $line) foreach ($labels as $label) if (str_contains(strtolower($line), $label) && preg_match_all('/(?:₹|rs\.?|inr)?\s*([0-9]+(?:,[0-9]{3})*(?:\.[0-9]{1,2})?)/i', $line, $m) && !empty($m[1])) return (float) str_replace(',', '', end($m[1]));
        return null;
    }
    ...
    {
        $start = null;
        foreach ($lines as $i => $line) if (preg_match('/description|item|product|particular|details/i', $line) && preg_match('/qty|quantity/i', $line) && preg_match('/rate|price|amount/i', $line)) { $start = $i + 1; break; }
        ...
        foreach (array_slice($lines, $start) as $line) {
            ...
            $items[] = ['description' => $description, 'hsn' => null, 'quantity' => $n[count($n)-3] ?? 1, 'unit' => null, 'rate' => $n[count($n)-2], 'discount' => null, 'tax_percent' => null, 'amount' => end($n), 'confidence' => null];
        }
        return $items;
    }
}
