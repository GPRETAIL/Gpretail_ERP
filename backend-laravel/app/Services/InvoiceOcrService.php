...
class InvoiceOcrService
{
    ...
    {
        $key = (string) config('services.ocr_space.key');
        if ($key === '') {
            throw new RuntimeException('Invoice OCR is not configured. Set OCR_SPACE_API_KEY.');
        }

        $response = Http::timeout((int) config('services.ocr_space.timeout', 120))
            ->connectTimeout((int) config('services.ocr_space.connect_timeout', 10))
        ...
        $text = collect($payload['ParsedResults'] ?? [])
            ->map(fn (array $page) => trim((string) ($page['ParsedText'] ?? '')))
            ->filter()->implode("\n");
    ...
    private function fileType(UploadedFile $file): string
    {
        return match (strtolower($file->getClientOriginalExtension())) {
            'pdf' => 'PDF', 'jpg', 'jpeg' => 'JPG', 'png', 'webp' => 'PNG', default => 'JPG'
        };
    }
    ...
    private function parseInvoice(string $text): array
    {
        ...
        return [
            ...
            'invoice' => [
                'number' => $this->match($text, '/(?:invoice|inv|bill)\s*(?:no|number|#)?\s*[:\-]?\s*([A-Z0-9][A-Z0-9.\/_-]{2,})/i', 1),
    ...
    private function items(array $lines): array
    {
        ...
        foreach ($lines as $i => $line) {
            if (preg_match('/description|item|product|particular|details/i', $line) && preg_match('/qty|quantity/i', $line) && preg_match('/rate|price|amount/i', $line)) { $start = $i + 1; break; }
        }
